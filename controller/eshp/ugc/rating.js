...
import RatingModel from '../../../models/eshp/ugc/rating';
...
class Rating {
    ...
    async initData(restaurant_id) {
        try {
            ...
            if (status) {
                console.log('init rate data success');
            }
        } catch(err) {
            console.log('init rate data failed');
            throw new Error(err);
        }
    }
    ...
    async getRatings(req, res, next) {
        ...
        try {
            ...
            console.log('get rate list failed', err);
            res.send({
                status: 0,
                type: 'ERROR_DATA',
    ...
    async getTags(req, res, next) {
        ...
        try {
            ...
            console.log('get rate tag failed', err);
            res.send({
                status: 0,
                type: 'ERROR_DATA',
                message: '未找到当前餐馆的评论数据'
