...
import {Food as FoodModel, Menu as MenuModel} from '../../../models/eshp/shopping/food';
...
class Food extends BaseComponent {
    ...
    async initData(restaurant_id) {
        for (let i = 0; i < this.defaultData.lenght; i++) {
            let category_id;
            try {
                category_id = await this.getId('category_id');
            } catch(err) {
                console.log('get category_id failed');
                throw new Error(err);
            }
            ...
            try {
                await newFood.save();
                console.log('init food data success');
            } catch(err) {
                console.log('init food data failed');
                throw new Error(err);
            }
        }
    }
    ...
    async getCategory(req, res, next) {
        ...
        try {
            ...
            console.log('get restaurant food kind failed');
            res.send({
                status: 0,
                type: 'ERROR_GET_DATA',
                message: '获取数据失败'
    ...
    async addCategory(req, res, next) {
        ...
        form.parse(req, async (err, fields, files) => {
            try {
                if (!fields.name) {
                    throw new Error('must fill food kind name');
    ...
    async addFood(req, res, next) {
        ...
        form.parse(req, async (err, fields, files) => {
            try {
                if (!fields.name) {
                    throw new Error('must fill food name');
                } else if (!fields.image_path) {
                    throw new Error('must upload food image');
                } else if (!fields.specs.length) {
                    throw new Error('at least one kind size');
                } else if (!fields.category_id) {
                    throw new Error('food kind id error');
                } else if (!fields.restaurant_id) {
                    throw new Error('restaurant id error');
                }
            } catch(err) {
            ...
            try {
                ...
                console.log('get item_id failed');
                res.send({
                    status: 0,
                    type: 'ERROR_DATA',
                    message: '添加食品失败'
            ...
            try {
                ...
                console.log('add specs failed', err);
                res.send({
                    status: 0,
                    type: 'ERROR_DATA',
                    message: '添加食品失败'
            ...
            try {
                ...
                console.log('save food to database failed', err);
                res.send({
                    status: 0,
                    type: 'ERROR_DATA',
                    message: '添加食品失败'
    ...
    async getSpecfoods(fields, item_id) {
        ...
        if (fields.specs.length < 2) {
            ...
            try {
                ...
                throw new Error('get sku_id food_id failed');
            }
            specfoods.push({
    ...
    async getMenu(req, res, next) {
        ...
        if (!restaurant_id || !Number(restaurant_id)) {
            console.log('get restaurant id param error');
            res.send({
                status: 0,
                type: 'ERROR_PARAMS',
                message: '餐馆ID参数错误',
        ...
        try {
            ...
            console.log('get food data failed', err);
            res.send({
                status: 0,
                type: 'GET_DATA_ERROR',
                message: '获取食品数据失败'
    ...
    async getMenuDetail(req, res, next) {
        ...
        if (!category_id || !Number(category_id)) {
            console.log('get menu detail id param error');
            res.send({
                status: 0,
                type: 'ERROR_PARAMS',
                message: 'Menu ID参数错误',
        ...
        try {
            ...
            console.log('get Menu detail failed', err);
            res.send({
                status: 0,
                type: 'GET_DATA_ERROR',
                message: '获取Menu详情失败'
    ...
    async getFoodsCount(req, res, next) {
        ...
        try {
            ...
            console.log('get food count failed', err);
            res.send({
                status: 0,
    ...
    async updateFood(req, res, next) {
        ...
        form.parse(req, async (err, fields, files) => {
            if (err) {
                console.log('get food info form error', err);
                res.send({
                    status: 0,
                    type: 'ERROR_FORM',
                    message: '表单信息错误'
            ...
            try {
                if (!name) {
                    throw new Error('food name error');
                } else if (!item_id || !Number(item_id)) {
                    throw new Error('food ID error');
                } else if (!category_id || !Number(category_id)) {
                    throw new Error('food kind ID error');
                } else if (!image_path) {
                    throw new Error('food image path error');
                }
                const [specfoods, specifications] = await this.getSpecfoods(fields, item_id);
                let newData;
    ...
    async deleteFood(req, res, next) {
        ...
        if (!food_id || !Number(food_id)) {
            console.log('food_id param error');
            res.send({
                status: 0,
                type: 'ERROR_PARAMS',
                message: 'food_id参数错误'
        ...
        try {
            ...
            console.log('delete food failed', err);
            res.send({
                status: 0,
