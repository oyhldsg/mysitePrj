...
import OrderModel from '../../../models/eshp/bos/order';
import CartModel from '../../../models/eshp/v1/cart';
...
class Order extends BaseComponent {
    ...
    async postOrder(req, res, next) {
        ...
        form.parse(req, async (err, fields, files) => {
            if (err) {
                console.log('formidable decode error', err);
                res.send({
                    status: 1,
                    message: '下单失败'
                });
            ...
            const orderObj = {
                basket: {
                    ...
                    packing_fee: {
                        ...
                        price: cartDetail.cart.extra[0].prict,
    ...
    async getDetail(req, res, next) {
        ...
        try {
            if (!user_id || !Number(user_id)) {
                ...
                throw new Error('order_id param error');
            }
        } catch(err) {
        ...
        try {
            ...
            console.log('get order info failed', err);
            res.send({
                status: 0,
    ...
    async getAllOrders(req, res, next) {
        ...
        try {
            ...
            console.log('get order data failed', err);
            res.send({
                status: 0,
    ...
    async getOrdersCount(req, res, next) {
        ...
        try {
            ...
            console.log('get order count failed', err);
            res.send({
                status: 0,
